...
for label in ["A", "B", "C"]:
    ...
    with col1:
        label_positions[label+"_x"] = st.number_input(
            f"{label} 集合 X座標", min_value=-1.0, max_value=1.0, value=0.0, step=0.05)
            #f"集合 {label} X座標", min_value=-1.0, max_value=1.0, value=0.0, step=0.05)
    with col2:
        label_positions[label+"_y"] = st.number_input(
            f"{label} 集合 Y座標", min_value=-1.0, max_value=1.0, value=0.0, step=0.05)
...
if mode == "2 圓":
    ...
    with col1:
        setA_size = st.number_input("A集合 大小", min_value=0, value=30)
        setA_color = st.color_picker("A集合 顏色", "#FFAAC0")
    with col2:
        setB_size = st.number_input("B集合 大小", min_value=0, value=20)
        setB_color = st.color_picker("B集合 顏色", "#C6AFE9")
...
# =====================
    ...
    with col1:
        a_size = st.number_input("A集合 大小", min_value=0, value=40)
        a_color = st.color_picker("A集合 顏色", "#FFAAC0")
    with col2:
        b_size = st.number_input("B集合 大小", min_value=0, value=35)
        b_color = st.color_picker("B集合 顏色", "#C6AFE9")
    with col3:
        c_size = st.number_input("C集合 大小", min_value=0, value=30)
        c_color = st.color_picker("C集合 顏色", "#99CC99")
