...
# plt.rcParams['font.sans-serif'] = ['AppleGothic']       # macOS
# plt.rcParams['font.sans-serif'] = ['Noto Sans CJK JP'] # Linux/其他系統
...
# 2 圓或 3 圓都使用 A/B/C，C 可不填
label_positions = {}
for label in ["A", "B", "C"]:
    col1, col2 = st.sidebar.columns(2)
    with col1:
        label_positions[label+"_x"] = st.number_input(f"集合 {label} X座標", min_value=-1.0, max_value=1.0, value=0.0, step=0.05)
    with col2:
        label_positions[label+"_y"] = st.number_input(f"集合 {label} Y座標", min_value=-1.0, max_value=1.0, value=0.0, step=0.05)
...
if mode == "2 圓":
    ...
    if intersection > min(setA_size, setB_size):
        ...
        if not show_labels:
            ...
            # 調整標籤位置
            if v.set_labels[0]: v.set_labels[0].set_position((label_positions["A_x"], label_positions["A_y"]))
            if v.set_labels[1]: v.set_labels[1].set_position((label_positions["B_x"], label_positions["B_y"]))

        if chart_title.strip(): plt.title(chart_title)
        st.pyplot(fig)
...
# =====================
    ...
    if abc > min(ab, ac, bc):
        ...
        if not show_labels:
            ...
            if v.set_labels[0]: v.set_labels[0].set_position((label_positions["A_x"], label_positions["A_y"]))
            if v.set_labels[1]: v.set_labels[1].set_position((label_positions["B_x"], label_positions["B_y"]))
            if v.set_labels[2]: v.set_labels[2].set_position((label_positions["C_x"], label_positions["C_y"]))
