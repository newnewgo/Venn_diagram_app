...
.stTextInput, .stNumberInput, .stColorPicker, .stSlider, .stRadio {
...
label_font_size = st.sidebar.number_input("標籤字體大小（Set1/Set2/Set3）", min_value=5, max_value=100, value=8)
value_font_size = st.sidebar.number_input("區塊數字字體大小", min_value=5, max_value=100, value=40)
    ...
    if intersection > min(set1_size, set2_size):
        ...
        if show_values:
            for t in v.subset_labels:
                if t:
                    t.set_fontsize(value_font_size)
